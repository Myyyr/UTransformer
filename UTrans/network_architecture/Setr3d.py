...
class Setr3d_Module(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False):
        ...
        encodlayer = nn.TransformerEncoderLayer(d_model = d_model, nhead = 6, dim_feedforward=1536, dropout=0.1, activation='relu')
        self.transformers_0 = nn.TransformerEncoder(encoder_layer=encodlayer, num_layers=8)
        self.transformers_1 = nn.TransformerEncoder(encoder_layer=encodlayer, num_layers=8)
        self.transformers_2 = nn.TransformerEncoder(encoder_layer=encodlayer, num_layers=8)
    ...
    def forward(self, inputs):
        ...
        # Deep Supervision
        # ds0 = self.ds0_cls_conv(torch.reshape(skip_0.permute((1,0,2)), (bs, c*self.d_model, int(d/16), int(w/16), int(h/16))))
        # del skip_0
        # ds1 = self.ds1_cls_conv(torch.reshape(skip_1.permute((1,0,2)), (bs, c*self.d_model, int(d/16), int(w/16), int(h/16))))
        # del skip_1
        # ds2 = self.ds2_cls_conv(torch.reshape(skip_2.permute((1,0,2)), (bs, c*self.d_model, int(d/16), int(w/16), int(h/16))))
        ...
        # Deconv
        # result = self.transposeconv_stage3(torch.reshape(skip_2.permute((1,0,2)), (bs, c*self.d_model, int(d/16), int(w/16), int(h/16))))
